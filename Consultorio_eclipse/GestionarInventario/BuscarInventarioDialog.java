...
public class BuscarInventarioDialog {
	private JDialog ventana = null;  //  @jve:decl-index=0:visual-constraint="89,35"
	...
	public static final int DESCONTAR = 1;
	
	public BuscarInventarioDialog(int m) {
		seleccionado = "";
		cantidad = -1;
		idProd = -1;
		modo = m;
	...
	public void abrir(){
		...
		if(index>=0){	
			String x;
			if(modo == DESCONTAR){
				x = JOptionPane.showInputDialog(ventana, "Cantidad de unidades a descontar", "Descontar productos", JOptionPane.QUESTION_MESSAGE);
			}
			else{
				x = JOptionPane.showInputDialog(ventana, "Cantidad de unidades a agregar", "Agregar productos", JOptionPane.QUESTION_MESSAGE);
			}
			try{
				int val = Integer.parseInt(x);
				if(val<=0){
					JOptionPane.showMessageDialog(ventana, "La cantidad debe ser mayor a cero", "Descontar productos", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
				}
				else{
					int total = Integer.parseInt(tabla.getValueAt(index, 2).toString());
					if(total >= val || modo == AGREGAR){
						cantidad = val;
						idProd = Integer.parseInt(listamedi.get(index).toString());
						seleccionado = tabla.getValueAt(index, 1).toString();
						ventana.setVisible(false);
					}
					else{
						JOptionPane.showMessageDialog(ventana, "No hay suficientes unidades", "Descontar productos", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
					}
				}
			}
			catch(Exception e){
				e.printStackTrace();
				JOptionPane.showMessageDialog(ventana, "La cantidad a descontar debe ser un número válido", "Descontar productos", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
			}
		}
		else{
	...
	private JButton getB_cerrar() {
		if (b_cerrar == null) {
			...
			b_cerrar.setLocation(new Point(250, 350));
	...
	private JButton getB_abrir() {
		if (b_abrir == null) {
			b_abrir = new JButton();
			b_abrir.setLocation(new Point(70, 350));
			b_abrir.setFont(new Font("Dialog", Font.BOLD, 12));
			b_abrir.setText("Modificar");
			b_abrir.setMnemonic(KeyEvent.VK_M);
			b_abrir.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image087.png")));

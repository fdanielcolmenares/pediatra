...
public class VentanaAgregarConceptos {
	...
	public VentanaAgregarConceptos(JDesktopPane p, Autenticacion a){
		panel = p;
		autenticacion = a;
		lista = new ArrayList();
		idConcepto = -1;
	...
	public void guardar(){
		conceptos ccto = new conceptos(autenticacion);
		if(idConcepto == -1){
			ccto.buscarNuevoID();
			ccto.setDescripcion(t_descripcion.getText());
			String retorna = ccto.guardar();
			if(retorna.compareToIgnoreCase("Ok")==0){
				mostrarMensaje("Registro exitoso", MENSAJE);
				idConcepto = ccto.getId();
				actualizarConceptos();
				if(otraVentana!=null){
			...
			ccto.setId(idConcepto);
			ccto.setDescripcion(t_descripcion.getText());
			String retorna = ccto.actualizar();
			if(retorna.compareToIgnoreCase("Ok")==0){
	...
	public void seleccionaConcepto(){
		...
		if(index >= 0){
			...
			idConcepto = Integer.parseInt(lista.get(index).toString());
		}
	}
	...
	public JInternalFrame getVentana() {
		if (ventana == null) {
			...
			ventana.setTitle("Conceptos");
			ventana.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
			ventana.setFrameIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image085.png")));

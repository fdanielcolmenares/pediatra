...
public class BuscarHistorias {
	...
	private boolean OptionChange = false;
	private int IDTable;
	
	public BuscarHistorias(JDesktopPane p) {
		panel = p;
		getVentana();
	...
	public BuscarHistorias(JFrame padre,boolean Option) {		
		//panel = padre;
		IDTable = 0;
		getDialog();
	...
	public void abrir(){
		...
		if(index>=0){
			int id = Integer.parseInt(tabla.getValueAt(index, 0).toString());
			
			CrearHistoria v = new CrearHistoria(panel);
	        GestorCrearHistoria g = new GestorCrearHistoria(v, autenticacion);
	        GestorCrearConsulta ges = new GestorCrearConsulta(v,autenticacion,panel);
	        v.setGestor(ges);
	        v.setGestor(g);
	...
	public void buscar(){
		...
		sql = "SELECT id, nombre FROM historias WHERE ";
	...
	public void buscarPorLetra(String letra){
		String sql = "SELECT id, nombre FROM historias "
		...
		if(con.conectar()){
			...
			try{
				...
				while(res!=null && res.next()){
					String id = "000000"+res.getString("id").toString();
	...
	public JInternalFrame getVentana() {
		if (ventana == null) {
			...
			ventana.setFrameIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image034.png")));
			ventana.setClosable(true);
			ventana.setTitle("Buscar Historias");
	...
	public JDialog getDialog(){
		if (dialog == null) {
			dialog = new JDialog();
			dialog.setSize(new Dimension(470, 440));
			dialog.setAlwaysOnTop(true);
			dialog.setModal(true);			
			dialog.setTitle("Buscar Historias");
	...
	public void setVisibleDialog(boolean inFlag){
			dialog.setVisible(inFlag);
	}
			...
			b_buscar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if(t_buscar.getText().length()>0){
						...
						if(dialog !=null){
							JOptionPane.showMessageDialog(dialog, "Debe ingresar un valor a buscar", "Error", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
						}
						else{
							JOptionPane.showMessageDialog(ventana, "Debe ingresar un valor a buscar", "Error", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
						}
			...
			b_abrir.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					abrir();
						...
						int index = tabla.getSelectedRow();
						if(index>=0){
							IDTable = Integer.parseInt(tabla.getValueAt(index, 0).toString());
							DialogDispose();
						}
					}
	...
	private JRadioButton getChk_id() {
		if (chk_id == null) {
			...
			chk_id.setSize(new Dimension(139, 21));
			chk_id.setLocation(new Point(202, 55));
			chk_id.setText("Número de Historia");
	...
	private JRadioButton getChk_nombre() {
		if (chk_nombre == null) {
			...
			chk_nombre.setSize(new Dimension(147, 21));
			chk_nombre.setText("Nombres o Apellidos");
			chk_nombre.setSelected(true);
			chk_nombre.setLocation(new Point(49, 55));
	...
	public void ChangeOptionChange(boolean inFlag){
			this.OptionChange = inFlag;
				if(inFlag){
						b_abrir.setText("Agregar");						
						b_abrir.setLocation(new Point(170, 350));

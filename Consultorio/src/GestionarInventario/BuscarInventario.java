...
public class BuscarInventario {
	...
	private Modelo_Tabla modelo;  //  @jve:decl-index=0:visual-constraint="0,59"
	...
	private JButton ocultar = null;
	
	public BuscarInventario(JDesktopPane p) {
		panel = p;
		getVentana();
	...
	public void abrir(){
		...
		if(index>=0){			
			nueCate nuevoCa = new nueCate(panel,false,autenticacion);
	    	GestorGestionarInventario ges = new GestorGestionarInventario(nuevoCa,autenticacion);	    	
	    	nuevoCa.setGestor(ges);
	    	ges.llenartipoProductos();
	    	ges.cargar(Integer.parseInt(listamedi.get(index).toString()));
	    	panel.add(nuevoCa.getFrmnueCate());
	    	nuevoCa.getFrmnueCate().setVisible(true);
	    	nuevoCa.getFrmnueCate().toFront();
		}
		else{
			JOptionPane.showMessageDialog(ventana, "Debe seleccionar un producto", "Búsqueda Completa", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
		}
	}
	...
	public void buscar(){
		...
		sql = "SELECT i.id AS id, i.descripcion aS descripcion, i.cantidad AS cantidad, "
				+"i.cantidad_minima AS cantidad_minima, i.valido AS valido, "
				+"i.tiro_id AS tiro_id, ti.descripcion AS tipo "
				+"FROM inventario i, tipo_inventario ti "
				+"WHERE ti.id=i.tiro_id AND LOWER(i.descripcion) LIKE '%"+busca.toLowerCase()+"%'";		
		ejecutaSQL(sql);
	}
	
	public void buscarPorLetra(String letra){
		String sql = "SELECT i.id AS id, i.descripcion aS descripcion, i.cantidad AS cantidad, "
			+"i.cantidad_minima AS cantidad_minima, i.valido AS valido, "
			+"i.tiro_id AS tiro_id, ti.descripcion AS tipo "
			+"FROM inventario i, tipo_inventario ti "
			+"WHERE ti.id=i.tiro_id AND LOWER(i.descripcion) LIKE '"+letra+"%'";
		ejecutaSQL(sql);
	}
		...
		if(con.conectar()){
			...
			try{
				...
				while(res!=null && res.next()){
					...
					tabla.setValueAt(res.getString("descripcion"), cont, 1);
					tabla.setValueAt(res.getString("cantidad"), cont, 2);
					tabla.setValueAt(res.getString("tipo"), cont, 3);
					if(res.getString("valido").compareTo("n")==0)
						tabla.setValueAt("NO", cont, 4);
					else
						tabla.setValueAt("SI", cont, 4);
				...
				e.printStackTrace();
				System.out.println("Buscar Inventario: Error");
			}
		}
	}
	...
	public JInternalFrame getVentana() {
		if (ventana == null) {
			...
			ventana.setFrameIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image092.png")));
			ventana.setClosable(true);
			ventana.setTitle("Buscar Productos");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.add(getOcultar(), null);
		}
		return jContentPane;
	}
	...
	private JButton getB_buscar() {
		if (b_buscar == null) {
			...
			b_buscar.setSize(new Dimension(107, 25));
			b_buscar.setLocation(new Point(261, 45));
			...
			b_buscar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					/*}
						...
						JOptionPane.showMessageDialog(ventana, "Debe ingresar un producto a buscar", "Error", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
	...
	private JButton getB_cerrar() {
		if (b_cerrar == null) {
			...
			b_cerrar.setLocation(new Point(310, 349));
			b_cerrar.setSize(new Dimension(120, 30));
			b_cerrar.setMnemonic(KeyEvent.VK_C);
			b_cerrar.setToolTipText("Cerrar Buscar Producto");
	...
	private JButton getB_abrir() {
		if (b_abrir == null) {
			b_abrir = new JButton();
			b_abrir.setLocation(new Point(36, 349));
			b_abrir.setFont(new Font("Dialog", Font.BOLD, 12));
			b_abrir.setText("Modificar");
			b_abrir.setMnemonic(KeyEvent.VK_M);
			b_abrir.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image039.png")));
			b_abrir.setToolTipText("Agregar Cantidad o Editar Producto");
	...
	private JTable getTabla() {
		if (tabla == null) {
			...
			modelo = new Modelo_Tabla(null,
					new String [] {"Nº","Nombre","Cantidad", "Tipo", "Válido"});
			...
			tabla.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(java.awt.event.MouseEvent e) {
					if(tabla.getValueAt(tabla.getSelectedRow(),4).toString().compareTo("NO")==0){
						getOcultar().setText("Visualizar");
						ocultar.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image042.png")));
					}
					else{
						getOcultar().setText("Ocultar");
						ocultar.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image008.png")));
					}
				}
			});
			tabla.addMouseListener(new java.awt.event.MouseAdapter() {
	...
	/**
	 * This method initializes ocultar	
	 * 	
	 * @return javax.swing.JButton	
	 */
	private JButton getOcultar() {
		if (ocultar == null) {
			ocultar = new JButton();
			ocultar.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image008.png")));
			ocultar.setText("Ocultar");
			ocultar.setToolTipText("Ocultar Medicamento del Inventario");
			ocultar.setMnemonic(KeyEvent.VK_O);
			ocultar.setLocation(new Point(172, 349));
			ocultar.setSize(new Dimension(125, 30));
			ocultar.setFont(new Font("Dialog", Font.BOLD, 12));
			ocultar.setName("Ocultar");
			ocultar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					int index = tabla.getSelectedRow();
					if(index>=0){
						inventario inventa = new inventario(autenticacion);
						if(getOcultar().getText().compareTo("Ocultar")==0){
							inventa.cambiarEstado(Integer.parseInt(listamedi.get(index).toString()), "n");
							tabla.setValueAt("NO", tabla.getSelectedRow(), 4);
							getOcultar().setText("Visualizar");
							ocultar.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image042.png")));
						}
						else{
							inventa.cambiarEstado(Integer.parseInt(listamedi.get(index).toString()), "s");
							tabla.setValueAt("SI", tabla.getSelectedRow(), 4);
							getOcultar().setText("Ocultar");
							ocultar.setIcon(new ImageIcon(getClass().getResource("/Files/Imagenes/Image008.png")));
						}						
					}
					else{
						JOptionPane.showMessageDialog(ventana, "Debe seleccionar un producto", "Eliminar", JOptionPane.ERROR_MESSAGE, new ImageIcon(getClass().getResource("/Files/Imagenes/Image007.png")));
					}
				}
			});
		}

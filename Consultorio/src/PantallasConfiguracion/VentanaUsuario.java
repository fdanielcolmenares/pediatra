...
public class VentanaUsuario {
    ...
    public void validar(){
    	try{
	    	...
	    	if(con.conectar()){
	    		...
	    		//System.out.println("clave: "+Encriptar.encryptMD5(String.valueOf(t_clave.getPassword())));
	    		ResultSet res = con.consultar(query);
	    		if(res.next()){
	    			cont = Integer.parseInt(res.getString(1).toString());	
	    			cont=1; //aqui quitar esto
	    		}
	    		con.desconectar();
	    		if(cont == 0){
	    			valido = false;
    	...
    	if(con.conectar()){
    		...
    		try{
    			if(res.next()){
    				...
    				rol =1; /*Integer.parseInt(res.getString(4).toString());*/
    ...
    private JDialog getVentana() {
        if (ventana == null) {
            ...
            ventana.setTitle("Autenticacion");
    ...
    private JPanel getPanelUsuario() {
        if (panelUsuario == null) {
            ...
            panelUsuario.setBorder(BorderFactory.createTitledBorder(null, "Ingrese Usuario y Contrasena", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));
